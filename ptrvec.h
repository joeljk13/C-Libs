...
int ptrvec_init(struct ptrvec *ptrvec) NONNULL LEAF;

/* Sets all the pointers to NULL. */
void ptrvec_zero(struct ptrvec *ptrvec) NONNULL LEAF;

/* Appends ptr to the end of ptrvec. Returns 0 on success, -1 on failure. */
int ptrvec_push(struct ptrvec *ptrvec, void *ptr) NONNULL_AT(1) LEAF;
 ...
 * empty. */
void * ptrvec_pop(struct ptrvec *ptrvec) NONNULL LEAF;

/* Returns the last pointer in ptrvec. Assumes ptrvec is not empty. */
void * ptrvec_peek(struct ptrvec *ptrvec) NONNULL LEAF;
...
int ptrvec_contains(struct ptrvec *ptrvec, void *ptr) NONNULL_AT(1) LEAF;

/* Returns the index where ptr is. If ptr is not in ptrvec, returns
 * ptrvec->lengh. */
size_t ptrvec_find(struct ptrvec *ptrvec, void *ptr) NONNULL_AT(1) LEAF;

/* Resizes ptrvec to size. Any new pointers added are set to NULL. Returns 0 on
 * success, -1 on failure. */
int ptrvec_resize(struct ptrvec *ptrvec, size_t size) NONNULL LEAF;

/* Reserves enough memory for at least size pointers. Returns 0 on success, -1
 * on failure. */
int ptrvec_reserve(struct ptrvec *ptrvec, size_t size) NONNULL LEAF;

/* Removes all pointers outside the range [begin, end). Returns 0 on success,
 * -1 on failure. */
int ptrvec_slice(struct ptrvec *ptrvec, size_t begin, size_t end) NONNULL LEAF;

/* Frees the memory used by ptrvec. */
void ptrvec_free(struct ptrvec *ptrvec) NONNULL LEAF;

/* Frees the memory used by each pointer in ptrvec and the memory used by
 * ptrvec. */
void ptrvec_delete(struct ptrvec *ptrvec) NONNULL LEAF;
